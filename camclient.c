...
PointCamAtTarget(edict_t *ent)
{
    ...
    if (fabsf(fDifference) > ent->client->fAngleLag)
    {
        if (fDifference > 0)
        {
...
RepositionAtTarget(edict_t *ent, vec3_t vOffsetPosition)
{
    ...
    if (fabsf(trace.endpos[0]-ent->s.origin[0]) > ent->client->fXYLag)
    {
    ...
    if (fabsf(trace.endpos[1]-ent->s.origin[1]) > ent->client->fXYLag)
    {
    ...
    if (fabsf(trace.endpos[2]-ent->s.origin[2]) > ent->client->fZLag)
    {
