...
static void Chicken_MenuCreate			(void);
...
static void	Chicken_ReadIni				(void);
...
static void Chicken_ReadMapList			(void);
static void Chicken_MOTDLoad			(void);
...
//
void Chicken_CreateStatusBar(void)
{
	char teamBar[256];
...
static void Chicken_Fire(edict_t *ent)
{
	if (ent && ent->client)
	{
		...
		if (teams == 0 && !canDrop)
		{
			...
			{
				if (ent->client->ps.gunframe == 15 && (strlen(SOUND_FIRE_WEAPON) != 0 ))
...
qboolean Chicken_Pickup(edict_t *ent, edict_t *other)
{
	if (ent && ent->item && other && other->client)
	{
		...
		gi.bprintf(PRINT_HIGH, "%s has picked up the Chicken\n", other->client->pers.netname);
...
//
//
void Chicken_ResetScoreboard(void)
{
	int team = 0;
...
//
void Chicken_ScoreCheck(edict_t *self, edict_t *inflictor, edict_t *attacker, int mod)
{
...
//	You guessed it, when a player dies this is called. Puts up lame messages
//
qboolean Chicken_PlayerDie(edict_t *self, edict_t *inflictor, edict_t *attacker, int mod)
{
	if (chickenGame)
	{
		...
		if (self != attacker)
		{
			if (self && self->client)
			{
				...
				if (attacker && attacker->client)
				{
					...
					if (which < 0.50)
					{
						size_t msgOff = ((sizeof(killerKilled) / sizeof(char*)) * random());
						gi.bprintf (PRINT_MEDIUM, killerKilled[msgOff], attacker->client->pers.netname, self->client->pers.netname);
					}
					if (which >= 0.50)
					{
						size_t msgOff = ((sizeof(killer) / sizeof(char *)) * random());
						gi.bprintf (PRINT_MEDIUM,killer[msgOff], attacker->client->pers.netname);
					}

				}
				else
				{
					size_t msgOff = ((sizeof(killed) / sizeof(char *)) * random());
					gi.bprintf (PRINT_MEDIUM,killed[msgOff], self->client->pers.netname);
				}
			}
			else
...
static int Chicken_GameStatus(void *data, int itemId)
{
	...
	else
	{
		...
		if (ent->client) {
			...
			ent->client->displayMenu = false;	// Remove menu ready for next command
	...
	while (fgets(buffer, sizeof(buffer), f) != NULL)
	{
		...
		if (buffer[0] != '\t' && buffer[0] != ' ' && buffer[0] != '\n' && buffer[0] != '#' && buffer[0] != '[')
		{
			...
			if ((newLevel			= (LevelName *)gi.TagMalloc(sizeof(*newLevel),	TAG_GAME)) == NULL ||
				(newLevel->mapName	= (char      *)gi.TagMalloc((int)strlen(token) + 1,	TAG_GAME)) == NULL)
				exit(-1);
			else
			{
...
static void Chicken_MOTDLoad(void)
{
	...
	if (fgets(buffer, sizeof(buffer), f) != NULL)
	{
		if ((token = strchr( buffer, '\n' )) != NULL)
			*token = '\0';
		sprintf(motd5, "%-30.30s", buffer);
	}
	if (fgets(buffer, sizeof(buffer), f) != NULL)
	{
		if ((token = strchr( buffer, '\n' )) != NULL)
			*token = '\0';
		sprintf(motd6, "%-30.30s", buffer);
	}
	if (fgets(buffer, sizeof(buffer), f) != NULL)
	{
		if ((token = strchr( buffer, '\n' )) != NULL)
			*token = '\0';
		sprintf(motd7, "%-30.30s", buffer);
	}
	if (fgets(buffer, sizeof(buffer), f) != NULL)
	{
		if ((token = strchr( buffer, '\n' )) != NULL)
			*token = '\0';
		sprintf(motd8, "%-30.30s", buffer);
	}
	fclose (f);
}
//
...
//
// ----------------------------------------------------------------------------------------
//	Fallback function to guarantee that chicken exists somewhere
//
static void Chicken_EnsureExists(void)
{
...
{
	char bigString[2048], header[1024];
	int  x = 0;
...
{
	char buf[128], buf2[128], bigString[2048], header[1024];
	int  x = 0;
...
{
	// If you have the chicken but have already thrown it dont drop another
	if (deathmatch->value && chickenGame && ent->client && ent->client->pers.inventory[chickenItemIndex] > 0 && !ent->client->isThrowing)
	{
