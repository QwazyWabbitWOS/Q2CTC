		...
		else
		{
...

			{
				...
				case OPT_TOGGLE:
				{
					...
					newItem->type->toggle->selectedText = txt;
				}
				break;
				...
				case OPT_OPTION:
				{
					...
					while (i != -1)
					{
						...
						newItem->type->option->optionList = realloc(newItem->type->option->optionList,
							newItem->type->option->maxOptions * sizeof(int));
						if(newItem->type->option->optionList == NULL)
							abort();
				...
				case OPT_SELECT:
                    newItem->type->select->func = (int (*)(void *, int))(va_arg(marker, void*));
					break;
...
void Chicken_MenuDisplay(void* data)
{
	...
	if (ent->client->currentMenu)
	{
		...
		while (curItem)
		{
			int len = (curItem->text ? (int)strlen(curItem->text) : 0);
